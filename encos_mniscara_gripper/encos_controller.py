...
if __name__ == "__main__":
    import matplotlib.pyplot as plt
    controller1 = EncosCanController(com_port="ZLG_31F10005727_1", controller_name="test1")
    controller2 = EncosCanController(com_port="ZLG_31F10005727_1", controller_name="test2")

    controller1.initialize()
    controller2.initialize()

    pos1 = controller1.get_instantaneous_position().get("content")
    pos2 = controller2.get_instantaneous_position().get("content")

    print(f"Controller 1 position: {pos1} °, Controller 2 position: {pos2} °")

    time.sleep(3)

    controller1.set_target_position(pos_deg=pos1+180, spd_val=100)
    controller2.set_target_position(pos_deg=pos2-180, spd_val=100)
    ...
    # stamps = []
    # total_nums = 200*3
    # current_pos = controller.get_instantaneous_position().get("content")

    # while total_nums > 0:
        # if total_nums == 400:
        #     controller.set_target_position(pos_deg=current_pos+436, spd_val=200)
        # if total_nums == 350:
        #     controller.set_target_position(pos_deg=current_pos+360, spd_val=200)
    #     if total_nums == 400:
    #         controller.set_target_position(pos_deg=current_pos+360, spd_val=200)
    #     speed = controller.get_instantaneous_speed(quiet=True).get("content")
    #     spds.append(speed)
    ...
    # plt.plot(stamps, spds, linewidth=0.5, color="tab:blue")
    # plt.scatter(stamps, spds, s=1, color="tab:blue")
    # plt.xlabel("Time (s)")
    # plt.ylabel("Speed")
